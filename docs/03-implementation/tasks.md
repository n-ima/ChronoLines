...
- [x] TASK-104: TimelineGrid（1年ズーム）— 行・列2軸仮想化（列44px・行28px・人物列200px・オーバースキャン5）・sticky 人物列（名前+生没年併記+タグ色ドット最大4+ツールチップ）・年ヘッダー・cellValue に基づくセル描画（blank/alive/virtual の色+括弧書式の二重チャネル）・セルのホバーツールチップ・10倍数年の縦罫線と現在年の強調罫線
  ...
  - 証拠 (2026-08-12): timelineGridModel.test.ts 21/21・typecheck エラー0・npm test 294/294・
    Playwright 機械確認 21/21（家康 1600=57/1700=(157)/1500=空欄・前100生まれ西暦1年=100・
    sticky 人物列・罫線・1000人×3000年で DOM 1520セルに限定/rAF 平均43ms・クラッシュなし）。
    メモ: 寸法定数は仮想化のJS計算用に timelineGridModel.ts へ tokens.css と同値で定義
    （正はトークン表とコメント明記）。virtual ツールチップ文言はモックアップ優先
- [x] TASK-105: 人物フォームとタグピッカーと削除フロー（〔＋人物〕/行メニュー〔編集〕・名前*/生年*/生月日/没年月日/タグ・blur+送信時検証〔E-P-NAME-EMPTY・E-YEAR-FORMAT・E-YEAR-ZERO・E-P-DEATH-BEFORE-BIRTH・E-DAY-WITHOUT-MONTH〕・タグピッカー〔付与済みピル✕除去/登録済みから選択/新規入力+サジェスト〕・削除確認〔個人イベントあり→3択、なし→2択〕）
  ...
  - 証拠 (2026-08-18): personForm.test.ts 50件新規で npm test 344/344 pass・typecheck エラー0・
    Playwright 機械確認 35/35（家康登録→行追加・没年<生年拒否・前100/-100受理と0拒否・
    生年編集で全セル再計算・3択/2択削除の実データ検証・タグピッカー一式・screen-03/04 一致）。
    メモ: 検証ロジックは personFormModel.ts に分離。共通モーダル枠 Dialog.tsx を新設（106/113 で再利用）。
    没月日のみ指定は没年欄に E-YEAR-FORMAT を出す仕様に決定。コミット 6c48743
- [x] TASK-106: イベントフォーム（〔＋イベント〕/サイドパネル〔編集〕/年ヘッダー右クリック〔この年にイベント追加〕・イベント名*/年*/月日/メモ2000字/人物紐付け/タグ・検証〔E-E-NAME-EMPTY・E-YEAR-*・E-DAY-WITHOUT-MONTH〕・フォーム内〔削除〕+確認）
  - 対応要件: US-003 / 対応設計: ui-forms-dialogs.md 2章、screen-03
  - 完了条件: 実行時確認: 「関ヶ原の戦い」を1600年で登録できる。年空欄の登録はエラー。
    年ヘッダー右クリックから年が初期値で入る
  - 証拠 (2026-08-18): eventForm.test.ts 36件新規で npm test 380/380 pass・typecheck エラー0・
    Playwright 機械確認 38/38（関ヶ原1600登録・年空欄=E-YEAR-FORMAT・右クリックで年初期値・
    編集+フォーム内削除・screen-03 dlg-event 構造/トークン一致）。
    メモ: サイドパネル〔編集〕経路は openEditEvent として用意（UI配線は TASK-107）。
    日select はモックアップどおり常時活性（E-DAY-WITHOUT-MONTH がUI到達可能）。コミット 8e965ed
- [x] TASK-107: イベントレーンと選択列と年齢比較サイドパネル（年ヘッダー直下の固定レーン・列ごと最大2チップ+「+N」バッジ・タグ配色/既定アンバー・個人イベントの人物アイコン・チップ/バッジ/年ヘッダークリック→選択列強調+右パネル〔年見出し・イベント全件リスト〔展開でメモ・編集・削除〕・年齢比較リスト〔行順・セルと同じ色書式・行クリックでスクロール〕〕・Esc/×で解除）
  ...
  - 証拠 (2026-08-18): selectionModel.test.ts 18件新規で npm test 398/398 pass・typecheck エラー0・
    Playwright 機械確認 31/31（成功指標=関ヶ原1600選択→家康57・政宗33同一画面・没後(66)グレー・
    100件パネルスクロール到達・パネル〔編集〕→イベントフォーム・Esc/✕解除・screen-01 一致）。
    メモ: selcol罫線色/+Nバッジ背景の2トークンを tokens.css に追加（screen-01 のリテラルを
    トークン化。design-tokens.md へは軽微誤記としてコーディネーターが転記）。
    ダイアログ表示中の Esc は最前面優先で選択列維持。コミット 711c3b4
- [x] TASK-108: 10年ズーム（トグル〔1年|10年〕・decadeStart による列生成〔西暦1〜9の9年バケット・紀元前境界〕・集約セル〔いずれか alive→帯+開始年年齢、全て没後/未来→仮想帯+(開始年仮想年齢)、生没マーカー〕・イベントレーンは件数バッジ→パネルで年別グループ全件・切替時の中心年保持スクロール）
  ...
  - 証拠 (2026-08-18): decadeZoom.test.ts 23件新規で npm test 421/421 pass・typecheck エラー0・
    Playwright 機械確認 22/22（1000年で列数1000→100・中心年1600が往復で完全一致・
    前1000〜/1〜9 の境界ラベル一致・集約セル/生没マーカー/件数バッジ/年別グループのトークン実測）。
    メモ: ズーム切替時は選択列を解除（列の意味が変わるため）。10年セルのツールチップ・
    罫線ガイド・右クリックはモックアップどおり非表示。コミット e9acf64
- [x] TASK-109: 人物検索（検索ボックス・150msデバウンスで searchPersons・ヒット行の人物列強調・「k/n件」+〔前へ/次へ〕で scrollToIndex・ヒット0件は「該当なし」表示で表示は不変）
  - 対応要件: US-008 / 対応設計: ui-timeline-grid.md 6章
  - 完了条件: 実行時確認: 1000人中「家康」で検索→該当行が表示範囲に現れ強調される。
    該当なし語→「該当なし」が出て行は変化しない
  - 証拠 (2026-08-18): searchModel.test.ts 16件新規で npm test 437/437 pass・typecheck エラー0・
    Playwright 機械確認 16/16（1000人で「家康」→1/3件・可視範囲外の行へスクロール+行強調・
    前へ/次へ巡回・該当なしで行数/スクロール位置とも不変・トークン実測一致）。
    メモ: 前へ/次へボタンはモックアップに無いが設計6章の明示要求に従い補完。
    スクロール要求は単調増加トークンで渡しデータ変化では発火しない。コミット 79e314b
- [x] TASK-110: タグ絞り込み（〔タグ▼〕ドロップダウン・allTags をチェックボックス複数選択〔色ドット+人物n・イベントm件数〕・OR条件で行とイベント両方に適用・適用中は色付きピル表示+個別✕/〔すべて解除〕）
  ...
  - 証拠 (2026-08-18): tagFilterModel.test.ts 19件新規で npm test 456/456 pass・typecheck エラー0・
    Playwright 機械確認 35/35（戦国30行・2選択OR35行・合戦イベント10件のみ・ピル個別✕/
    すべて解除で復帰・ドロップダウン/ピルのトークン実測一致）。
    メモ: 検索は絞り込み後の表示行に照合（domain-logic.md 2章のパイプライン準拠）。
    データ変化で消えたタグは選択から自動除去。--shadow-dropdown をトークン化し
    design-tokens.md に転記済み。コミット d89f3a5
- [x] TASK-111: 並び替えと行ドラッグ&ドロップ（トグル〔生年順|手動〕・手動時のドラッグハンドル・Pointer Events 自前実装〔行高固定でドロップ位置=オフセット/行高〕・reorderPerson・並び順の永続化・〔生年順〕へワンクリック復帰〔personOrder 保持〕）
  - 対応要件: US-008 / 対応設計: ui-timeline-grid.md 6章、data-model.md 4章
  - 完了条件: 実行時確認: 行をドラッグで並び替え→リロード後も復元される。生年順に戻して
    再度手動にすると前回の手動順に復帰する
  - 証拠 (2026-08-18): rowDragModel.test.ts 20件新規で npm test 476/476 pass・typecheck エラー0・
    Playwright 機械確認 26/26（ドラッグで並び替え→personOrder 永続化+リロード復元・
    生年順復帰・再手動で前回手動順復帰・インジケータ/ハンドルのトークン実測）。
    メモ: 見た目はモックアップどおり select（「トグル」表記は同義）。タグ絞り込み中は
    可視並びが変わらないドロップを no-op とし隠れ行との相対順を黙って動かさない。コミット ea7c3c3
- [x] TASK-112: 表示範囲指定と空状態（〔開始年〕〔終了年〕入力・parseYearInput・空欄=自動でプレースホルダに自動値・不正入力はインラインエラー〔E-YEAR-FORMAT/E-YEAR-ZERO/E-RANGE-INVERTED〕・範囲内に該当なし→空グリッド+バナー・人物0件→中央の空状態表示〔＋人物を追加〕）
  ...
  - 完了条件: 実行時確認: 1543〜1636のデータで範囲指定なし→1543〜現在年。1590〜1620指定→
    ...
    開始>終了はエラーで適用されない
  - 証拠 (2026-08-18): rangeModel.test.ts 33件新規で npm test 509/509 pass・typecheck エラー0・
    Playwright 機械確認 38/38（autoRange 1543〜現在年・1590〜1620で31列・1200〜1300で
    空グリッド+情報バナー・反転/形式/0年エラーで不適用・空データの中央空状態→人物フォーム）。
    メモ: 範囲入力の確定は blur/Enter（途中値でグリッドが跳ぶのを防ぐ）。E-RANGE-INVERTED は
    両方手動指定時のみ判定。表示値は formatYear へ正規化。コミット fd95a9c
- [x] TASK-113: 年表管理（ツールバー左端の年表切替ドロップダウン〔1クリック切替〕・〔年表の管理...〕ダイアログ〔一覧=名前/人物数/イベント数・切替・名前変更・削除〕・〔＋新しい年表〕〔E-T-NAME-EMPTY〕・削除確認〔人物n人・イベントm件も削除される旨〕・最後の1つ削除→空の「年表1」自動作成）
  ...
  - 完了条件: 実行時確認: 「戦国」「幕末」を切り替えるとそれぞれの人物・イベントだけが
    ...
    空の「年表1」が作られる
  - 証拠 (2026-08-18): timelineManagerModel.test.ts 15件新規で npm test 524/524 pass・
    typecheck エラー0・Playwright 機械確認 43/43（戦国⇔幕末切替で相互不可視・削除は承諾時のみ・
    最後の削除→「年表1」自動作成を一覧/ストア/保存ファイル3層で確認・E-T-NAME-EMPTY・
    ダイアログのトークン実測一致）。メモ: 名前変更は行内インライン編集で補完。
    名前50字上限は maxLength で機械的防止（新エラーIDは作らない）。コミット 4caa4d0

## 周辺機能

- [x] TASK-201: エクスポートダイアログ（〔入出力〕→〔エクスポート〕タブ・範囲選択〔現在の年表のみ=timelines を1件に絞り activeTimelineId 差し替え/すべて〕・クライアント単独の Blob ダウンロード・ファイル名 chronolines-export-YYYYMMDD-HHmm.json）
  ...
  - 証拠 (2026-08-18): exportScope.test.ts 10件新規で npm test 534/534 pass・typecheck エラー0・
    Playwright 機械確認 27/27（current/all の実ダウンロードが storeSchema.parse 通過・
    current は active 1件+activeTimelineId 差し替え・ファイル名形式・screen-03 #dlg-io 一致）。
    メモ: 絞り込みは storeForExportScope として src/domain/export.ts に追加（buildExportPayload と
